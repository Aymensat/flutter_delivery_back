...
export const registerUser = async (req, res) => {
  try {
    ...
    if (req.body.location) {
      ...
      latitude = parseFloat(
        req.body["location[latitude]"] || req.body.latitude || 0
      );
      longitude = parseFloat(
        req.body["location[longitude]"] || req.body.longitude || 0
      );
    }
    ...
    console.log(
      `User registration successful: ${user.email} (ID: ${user._id})`
    );
...
export const getAllLivreurs = async (req, res) => {
  try {
    ...
    const livreursWithAddress = await Promise.all(
      livreurs.map(async (livreur) => {
        let address = null;
        if (
          livreur.location &&
          livreur.location.latitude &&
          livreur.location.longitude
        ) {
          address = await reverseGeocode(
            livreur.location.latitude,
            livreur.location.longitude
...
export const getAvailableLivreurs = async (req, res) => {
  try {
    ...
    // Add address field using reverse geocoding if available
    const livreursWithAddress = await Promise.all(
      availableLivreurs.map(async (livreur) => {
        let address = null;
        if (
          livreur.location &&
          livreur.location.latitude &&
...
export const rateLivreur = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Livreur rated successfully", livreur: livreurObj });
  } catch (error) {
...
export const updateOnlineStatus = async (req, res) => {
  try {
    ...
    if (typeof isOnline !== "boolean") {
      ...
      return res
        .status(400)
        .json({ message: "isOnline must be a boolean value" });
    }
    ...
    console.log(
      `[Backend] User ${userId} online status updated to ${isOnline}`
    );
