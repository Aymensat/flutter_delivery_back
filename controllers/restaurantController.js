...
import Food from "../models/Food.js"; // Add this line
...
export const getFoodsByRestaurant = async (req, res) => {
  try {
    ...
    // Find all food items where the 'restaurant' field matches the ID
    const foods = await Food.find({ restaurant: id });

    // Corrected check: check if the array is empty, not if it's null/undefined
    if (foods.length === 0) {
      return res
        .status(404) // Or 200 with an empty array if you prefer
        .json({ message: "No food items found for this restaurant" });
    }

    res.json(foods);
  } catch (err) {
    console.error("Error fetching foods for restaurant:", err);
    res.status(500).json({ message: err.message });
  }
};
...
export const createRestaurant = async (req, res) => {
  try {
    ...
    if (
      ...
      (restaurantData.latitude < -90 || restaurantData.latitude > 90)
    ) {
      return res
        .status(400)
    ...
    if (
      ...
      (restaurantData.longitude < -180 || restaurantData.longitude > 180)
    ) {
      return res
        .status(400)
    ...
    console.log(
      `Creating restaurant with coordinates: ${restaurantData.latitude}, ${restaurantData.longitude}`
    );
...
export const updateRestaurant = async (req, res) => {
  try {
    ...
    // Log the coordinates for debugging
    console.log(
      `Updating restaurant with coordinates: ${restaurantData.latitude}, ${restaurantData.longitude}`
    );
...
export const rateRestaurant = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Restaurant rated successfully", restaurant });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
