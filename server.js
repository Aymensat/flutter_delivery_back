...
app.use((req, res, next) => {
  ...
  console.log(
    `[${new Date().toISOString()}] ${req.method} ${req.path} - Query params:`,
    req.query
  );
  ...
  res.send = function (body) {
    ...
    console.log(
      `[${new Date().toISOString()}] ${req.method} ${req.path} - Response ${
        res.statusCode
      } (${responseTime}ms)`
    );
    ...
    completedDeliveries.forEach((delivery) => {
      if (delivery.deliveryFee) {
        ...
        if (!isNaN(fee)) {
          ...
          console.warn(
            `Invalid deliveryFee for delivery ${delivery._id}: ${delivery.deliveryFee}`
          );
        }
      }
      ...
      if (delivery.paymentCollected && delivery.totalAmount) {
        ...
        if (!isNaN(amount)) {
          ...
          console.warn(
            `Invalid totalAmount for delivery ${delivery._id}: ${delivery.totalAmount}`
          );
        }
      }
    });
...
async function reverseGeocode(lat, lon) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Reverse geocoding error: ${response.status} ${response.statusText}`
      );
      return null;
    }
...
app.get("/api/geocode/reverse", async (req, res) => {
  try {
    ...
    console.log(
      `Geocode request received for coordinates: ${latValue}, ${lonValue}`
    );
    ...
    if (address) {
      console.log(
        `Successfully geocoded ${roundedLat}, ${roundedLon} to "${address}"`
      );
      return res.json({ address });
    } else {
      console.warn(
        `Failed to geocode coordinates: ${roundedLat}, ${roundedLon}`
      );
...
app.get("/api/geocode/map-redirect", (req, res) => {
  ...
  if (!lat || !lon) {
    return res
      .status(400)
      .send("Latitude and longitude are required parameters");
  }
...
app.get("/api/geocode/search", async (req, res) => {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Forward geocoding error: ${response.status} ${response.statusText}`
      );
...
const fileUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (
      acceptedImageTypes.includes(file.mimetype) ||
    ...
    if (acceptedExtensions.includes(extension)) {
      console.log(
        `Accepting file ${file.originalname} based on extension ${extension}`
      );
      cb(null, true);
      return;
    }
    ...
    console.log(
      `Rejecting file ${file.originalname} with mimetype ${file.mimetype}`
    );
...
app.post("/api/upload", protect, (req, res) => {
  fileUpload.single("image")(req, res, (err) => {
    ...
    if (!req.file) {
      ...
      return res.status(400).json({
        error:
          "No file provided or invalid file type. Please upload a valid image file (jpg, png, gif, webp)",
      });
    }

    // Create a URL path for the uploaded file
    const filePath = `/uploads/${req.query.type || "general"}/${
      req.file.filename
    }`;
